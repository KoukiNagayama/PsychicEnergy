...
namespace nsPsychicEnergy
{
	...
	namespace nsPlayer
	{
		...
		public:
			...
			/// アニメーションスピードを設定。
			/// </summary>
			/// <param name="speed">アニメーションのスピード。</param>
			...
			/// プレイヤーの移動を無効化するか。
			/// </summary>
			/// <param name="disablePlayerMove"></param>
		...
		private:
			...
			/// <param name="eventName">設定したイベント名。</param>
		...
		private:
			...
			float					m_lastMoveSpeedY = 0.0f;						// 直前のY方向の移動速度。
			EffectEmitter*			m_effectEmitterWind = nullptr;					// 風のエフェクト。
			bool					m_disablePlayerMove = false;					// プレイヤーの動きの無効化。trueの場合に無効化する。
			SoundSource*			m_windSound = nullptr;							// 風の音
		};
	}
}
